...
public class ReadPcapFileWorker extends SwingWorker<List<String>, String> {
    ...
    private void readPcapDir(File inputPath, String outPath) {
        ...
        // File[] pcapFiles = inputPath.listFiles(file ->
    ...
    private void readPcapFile(String inputFile, String outPath) {
         ...
         * chunks.add(String.format("\t Ignored packets:%d %d ",
         * nDiscarded,(nTotal-nValid)));
         * chunks.add(String.format("PCAP duration %d seconds",((packetReader.
         * getLastPacket()- packetReader.getFirstPacket())/1000)));
         * chunks.add(DividingLine);
         * int singleTotal = flowGen.dumpLabeledFlowBasedFeatures(outPath, fullname+
